...
//Library is a 2d 8*[library size] array containing the 8 short int values of each bin value for the library of songs
...
void getDistances(short int** library, short int* sample, int* distances, int length)
{
	...
	for(k = 0; k < length ; k += 64)
	{
		...
		for(i = 0; i < 8; i++)
		{
			...
			//J indexes the current sample [column]
			for( j = 0; j < 64; j++)
				{
					...
					IOWR_32DIRECT(classifier_core_base,4,encode(j, library[i][(j+k)]));
				}
